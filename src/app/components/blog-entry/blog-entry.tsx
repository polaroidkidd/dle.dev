...
function recursiveList(line: Chapter) {
  ...
  if (title) {
    ...
    return (
      <li key={title}>
        <a
          href={`#${encodeStringToTURL(title)}`}>
          {title.replace(replaceNumberIndents(), "")}
        </a>

        {hasSubChapters

          ? <ol>{Object.values(rest).map(value => recursiveList(value))}</ol>
          : ''}
      </li>
    )
  }
        ...
        <ReactMarkdown
          ...
          renderers={{
            root: ({ children }) => {
              ...
              const outline = children.reduce((tableOfContents: any, { key, props }: { key: string, props: any }) => {
                ...
                if (tableOfContents !== undefined) {
                  tableOfContents = insertChapterIntoTableOfContents(tableOfContents, chapterLocationByIndex, chapterTitle)
                }
                return tableOfContents;
              }, {});
              ...
              const author = children[2].props.children.map((child: any) => {
                return <p><strong>{child.props.children[0].props.value}</strong></p>
              })
            ...
            heading: ({ level, children }) => {
              const anchorName = encodeStringToTURL(children[0].props.children.toString().replace(replaceNumberIndents(), ""))

              switch (level) {
                case 1: {
                  return <h1><a id={`${anchorName}`}>{children}</a></h1>
                }
                case 2: {
                  return <h2><a id={`${anchorName}`}>{children}</a></h2>
                }
                case 3: {
                  return <h3><a id={`${anchorName}`}>{children}</a></h3>
                }
                case 4: {
